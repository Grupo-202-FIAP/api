...
public class UserRepositoryAdapter implements UserRepositoryPort {
    ...
    public User save(User user) {
        logger.info("[Repository][User] Iniciando persistência do usuário: tipo={}, email={}", user instanceof Employee ? "Employee" : "Customer",
                user.getEmail());
        ...
        logger.info("[Repository][User] Usuário salvo com sucesso: id={}, tipo={}", userSaved.getId(),
                userSaved instanceof Employee ? "Employee" : "Customer");
    ...
    public User findByCpf(String cpf, UserRole role) {
        ...
        if (role != UserRole.ROLE_GUEST && role != UserRole.ROLE_CUSTOMER) {
            final EmployeeEntity employee = this.employeeEntityRepository.findByCpf(cpf).orElseThrow(() -> {
                logger.warn("[Repository][User] Funcionário não encontrado por CPF={}", cpf);

                return new FastFoodException("Employee not found with CPF: " + cpf, "Employee Not Found", HttpStatus.NOT_FOUND);
            });
            user = EmployeeMapper.toDomain(employee);
        } else {
            final CustomerEntity customer = this.customerEntityRepository.findByCpf(cpf).orElseThrow(() -> {
                logger.warn("[Repository][User] Cliente não encontrado por CPF={}", cpf);

                return new FastFoodException("Customer not found with CPF: " + cpf, "Customer Not Found", HttpStatus.NOT_FOUND);
            });
    ...
    public User findByEmail(String email, UserRole role) {
        ...
        User user = null;
        if (role != UserRole.ROLE_GUEST && role != UserRole.ROLE_CUSTOMER) {
            final EmployeeEntity employee = this.employeeEntityRepository.findByEmail(email).orElseThrow(() -> {
                logger.warn("[Repository][User] Funcionário não encontrado por email={}", email);

                return new FastFoodException("Employee not found with Email: " + email, "Employee Not Found", HttpStatus.NOT_FOUND);
            });
            user = EmployeeMapper.toDomain(employee);
        } else {
            final CustomerEntity customer = this.customerEntityRepository.findByEmail(email).orElseThrow(() -> {
                logger.warn("[Repository][User] Cliente não encontrado por email={}", email);

                return new FastFoodException("Customer not found with Email: " + email, "Customer Not Found", HttpStatus.NOT_FOUND);
            });
            user = CustomerMapper.toDomain(customer);
        }
        logger.info(LOG_USUARIO_ENCONTRADO, user.getId(), user.getEmail());
    ...
    public User findById(UUID id) {
        ...
        final User user = customerEntityRepository.findById(id).map(CustomerMapper::toDomain).map(User.class::cast)
                .or(() -> employeeEntityRepository.findById(id).map(EmployeeMapper::toDomain).map(User.class::cast)).orElseThrow(() -> {
                    logger.warn("[Repository][User] Nenhum usuário encontrado com ID={}", id);
                    return new FastFoodException("User not found with ID: " + id, "User Not Found", HttpStatus.NOT_FOUND);
                });
